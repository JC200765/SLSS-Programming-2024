...
class Rayquaza(Pokemon):
    def __init__(self, name="Rayquaza"):
        super().__init__()

        self.name = name
        self.id = 384
        self.type = "Dragon", "Flying"
        self.actual_cry = "Roarrr"

    def Dragon_Ascent(self, defender: Pokemon) -> str:
        response = f"{self.name} used Dragon Ascent on {defender.name}"
        if defender.type.lower() in ["fairy, ice"]:
            response = response + "it was not effective"
        if defender.type.lower() in ["ground, water"]:
